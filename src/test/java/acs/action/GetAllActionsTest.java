...
class GetAllActionsTest {
	...
	@Test
	public void testGetAllActionsNonAdminUserGivenDBWithFiveElementsReturnStatusDifferentFrom2xx() throws Exception {
		// GIVEN the server is up
		
		// create non-admin users
		UserBoundary playerBoundary = UserTestUtil.userPost(this.restTemplate, this.url, UserTestUtil.createPlayer());
		UserBoundary managerBoundary = UserTestUtil.userPost(this.restTemplate, this.url, UserTestUtil.createManager());
		...
		map.setType("map");
		map.setActive(true);
		map = ElementTestUtil.elementPost(this.restTemplate, this.url, map, managerBoundary);
		...
		for (int i = 0; i < 5; i++) {
			actionBoundary.setActionAttributes(ActionTestUtil.randFeedingAreaAttributes(managerBoundary, map));
			storedElements.add(ActionTestUtil.postAction(this.restTemplate, this.url, actionBoundary));
		}

		// WHEN I GET /{userDomain}/{userEmail} with non-admin user the server throws an exception
		assertThrows(Exception.class, () -> ActionTestUtil.getAllActions(this.restTemplate, this.url, playerBoundary));
		assertThrows(Exception.class, () -> ActionTestUtil.getAllActions(this.restTemplate, this.url, managerBoundary));
	}
