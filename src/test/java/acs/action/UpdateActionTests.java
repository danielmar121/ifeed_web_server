...
class UpdateActionTests {
	...
	void update_food_bowl_by_invoke_action() {
		...
		foodBowl.setElementAttributes(updateFoodBowl(foodBowl.getElementId(), playerBoundary));
		...
		assertThat(foodBowl).usingRecursiveComparison().isEqualTo(updatedFoodBowl);
	}
	...
	void update_water_bowl_by_invoke_action() {
		...
		waterBowl.setElementAttributes(updateWaterBowl(waterBowl.getElementId(), playerBoundary));
		...
		assertThat(waterBowl).usingRecursiveComparison().isEqualTo(updatedWaterBowl);
	}
	...
	@Test
	void update_food_bowl_and_water_bowl_by_invoke_action() {
		...
		IntStream.range(0, updatedBowls.length)
				.forEach(i -> updatedBowls[i].setElementAttributes(updateBowl(updatedBowls[i], playerBoundary)));

		// Then the DB would contain the updated Food Bowl and Water Bowl:
		ElementBoundary[] expectedUpdatedBowls = getChildrenOf(feedingArea);

		assertThat(updatedBowls).usingElementComparatorOnFields("elementId", "type", "name", "active",
				"createdTimestamp", "createdBy", "location", "elementAttributes")
				.containsExactlyInAnyOrder(expectedUpdatedBowls);
	}

	@Test
	void testUpdateFoodbowlAndWaterbowlByNonManagerUserGivenNonEmptyDBReturnsStatusDifferentFrom2xx() {
		// Given a DB with a single Feeding Area and a Food Bowl and a Water Bowl:
		// Given non-manager users
		UserBoundary playerBoundary = createPlayer();
		...
		// When I PUT these foodbowl and waterbowl with a new random element *by
		// non-manager user*, the server throws exceptions
		ElementBoundary[] bowls = getChildrenOf(feedingArea);
		IntStream.range(0, bowls.length)
				.forEach(i -> assertThrows(Exception.class, () -> ElementTestUtil.updateElement(this.restTemplate,
						this.url, ElementTestUtil.randElementBoundary(), bowls[i], playerBoundary)));
		IntStream.range(0, bowls.length)
				.forEach(i -> assertThrows(Exception.class, () -> ElementTestUtil.updateElement(this.restTemplate,
						this.url, ElementTestUtil.randElementBoundary(), bowls[i], adminBoundary)));
	}
	...
	private Map<String, Object> feedingAreaActionAttributes() {
		...
		actionAttributes.put("elementName", "feed_area_" + randString(5));
		actionAttributes.put("elementLat", r.nextDouble());
		actionAttributes.put("elementLng", r.nextDouble());
		actionAttributes.putAll(feedingAreaAttributes());
		return actionAttributes;
	}

	private Map<String, Object> feedingAreaAttributes() {
		Map<String, Object> attributes = new TreeMap<>();
		attributes.put("fullFoodBowl", 0);
		attributes.put("fullWaterBowl", 0);
		return attributes;
	}

	private Map<String, Object> foodBowlActionAttributes() {
		Random r = new Random();
		Map<String, Object> actionAttributes = new LinkedHashMap<>();
		actionAttributes.put("managerDomain", this.appDomain);
		actionAttributes.put("managerEmail", this.managerEmail);
		actionAttributes.put("elementName", "food_bowl_" + randString(5));
		actionAttributes.put("elementLat", r.nextDouble());
		actionAttributes.put("elementLng", r.nextDouble());
		actionAttributes.putAll(foodBowlAttributes());
		return actionAttributes;
	}
	...
	private Map<String, Object> foodBowlAttributes() {
		...
		actionAttributes.put("brand", ElementTestUtil.foodBrands[r.nextInt(ElementTestUtil.foodBrands.length)]);
		actionAttributes.put("animal", ElementTestUtil.animals[r.nextInt(ElementTestUtil.animals.length)]);
		actionAttributes.put("weight", r.nextInt(500));
		actionAttributes.put("lastFillDate",
				LocalDate.now().minusDays(new Random().nextInt(ElementTestUtil.maxDays)).toString());
		return actionAttributes;
	}
	...
	private Map<String, Object> waterBowlActionAttributes() {
		...
		actionAttributes.put("elementLat", r.nextDouble());
		actionAttributes.put("elementLng", r.nextDouble());
		actionAttributes.putAll(waterBowlAttributes());
		return actionAttributes;
	}
	...
	private Map<String, Object> waterBowlAttributes() {
		...
		Map<String, Object> actionAttributes = new TreeMap<>();
		actionAttributes.put("state", r.nextBoolean());
		actionAttributes.put("waterQuality",
				ElementTestUtil.waterQualities[r.nextInt(ElementTestUtil.waterQualities.length)]);
		return actionAttributes;
	}
	...
	private void postManagerAndMap() {
		...
		ElementBoundary mapBoundary = new ElementBoundary(null, "map", "map", true, new Date(),
