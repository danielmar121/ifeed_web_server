...
public class GetAllElementsTest {
	...
	@Test
	public void testGetAllElementsByAdminAndGetException() throws Exception
	{
		// When get all elements
		// Given they are Player or Admin
		// Throw Exception
		UserBoundary adminBoundary = UserTestUtil.adminPost(this.restTemplate, this.url);	
		assertThrows(RuntimeException.class, () -> ElementTestUtil.getAllElements(this.restTemplate, this.url, adminBoundary));
	}
	...
	@Test
	public void testGetAllElementsByPlayerWith20ActiveElementsInDatabaseAnd3NotActiveReturnArraysOf20Messages()
			throws Exception {
		// GIVEN the server is up
		// do nothing
		UserBoundary managerBoundary = UserTestUtil.userPost(restTemplate, url, UserTestUtil.createManager());
		...
		// GIVEN database which contains 20 active elements and 3 not active elements
		List<ElementBoundary> activeElementsInDB = new ArrayList<>();
		IntStream.range(0, 20).forEach(i -> activeElementsInDB.add(ElementTestUtil.elementPost(this.restTemplate,
				this.url, ElementTestUtil.randElementBoundaryWithSpecificActive(true), managerBoundary)));
		...
		assertThat(activeElementsInDB).usingRecursiveFieldByFieldElementComparator().containsExactlyInAnyOrder(rv);
	}
