...
public class GetElementTest {
	...
	@Test
	public void testGetInactiveSingleElementFromServerWillReturnEmptyArray() throws Exception
	{
		//GIVEN I GET an element
		// WHILE it is inactive
		// return empty array
		
		// create manager -> create inactive father element -> 
		// create child element and bind them -> get parents from child -> assert empty array
		...
		ElementBoundary father = ElementTestUtil.randElementBoundary();
		father.setActive(false);
		ElementBoundary child = ElementTestUtil.randElementBoundary();
		
		ElementBoundary fatherElement = ElementTestUtil.elementPost(this.restTemplate, this.url, father, managerBoundary);
		ElementBoundary childElement = ElementTestUtil.elementPost(this.restTemplate, this.url, child, managerBoundary);

		ElementTestUtil.bindElement(this.restTemplate, this.url, fatherElement, childElement);
		
		
		ElementBoundary[] parents = ElementTestUtil.playerGetParentOf(this.restTemplate, this.url, childElement, playerBoundary);
		ElementBoundary[] children = ElementTestUtil.getChildrenOf(this.restTemplate, this.url, fatherElement);
		

		assertTrue(children[0].getElementId().equals(childElement.getElementId()));
